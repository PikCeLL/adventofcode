...
class Day1 : IDailyPuzzle {
    ...
    override fun getResultPuzzle1(input: String): Long {
        return input.lineSequence().map { line -> (line.find { it.isDigit() }.toString() + line.findLast { it.isDigit() }).toLong() }.sum()
    }
    ...
    override fun getResultPuzzle2(input: String): Long {
        ...
        return input.lineSequence().map { line -> (literal2numeral(line.findAnyOf(digitList)!!.second) + literal2numeral(line.findLastAnyOf(digitList)!!.second)).toLong() }.sum()
    }
